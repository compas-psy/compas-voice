...
android {
    ...
    defaultConfig {
        ...
        // в BuildConfig.ANALYTICS_INGEST_SECRET внутри APK — а BuildConfig в
        // apk — не тайна: `unzip`/`apktool`/`jadx` любого скачанного APK
        // достают его за минуты, он один и тот же во всех установках. Это НЕ
        // аутентификация устройства и не защита от того, кто целенаправленно
        // ищет секрет в APK, — так делать нельзя, если нужна такая защита.
        // Это фильтр случайного постороннего трафика на приёмник ПРАКТИКИ (не
        // угадать URL+секрет наугад), не более. Правильная замена на будущее,
        // когда/если это станет важно: не общий статический секрет на все
        // инсталляции, а токен на устройство — выданный сервером в обмен на
        // аттестацию (Play Integrity API или аналог), короткоживущий и с
        // ротацией, а не зашитый в бинарник раз и навсегда.
        buildConfigField(
            "String",
